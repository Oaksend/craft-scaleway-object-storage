<?php

declare(strict_types=1);

namespace Scaleway\ObjectStorage;

use Aws\Handler\GuzzleV6\GuzzleHandler;
use Craft;
use craft\flysystem\base\FlysystemFs;
use craft\helpers\App;
use craft\helpers\DateTimeHelper;
use DateTime;
use League\Flysystem\AwsS3V3\AwsS3V3Adapter;
use League\Flysystem\AwsS3V3\PortableVisibilityConverter;
use League\Flysystem\FilesystemAdapter;
use League\Flysystem\Visibility;
use League\MimeTypeDetection\FinfoMimeTypeDetector;

/**
 * Class Scaleway
 *
 * @property mixed $settingsHtml
 * @property string $rootUrl
 */
class ScalewayFs extends FlysystemFs
{
    /**
     * @var string Subfolder to use
     */
    public string $subfolder = '';

    /**
     * @var string AWS key ID
     */
    public string $keyId = '';

    /**
     * @var string AWS key secret
     */
    public string $secret = '';

    /**
     * @var string Bucket to use
     */
    public string $bucket = '';

    /**
     * @var string Region to use
     */
    public string $region = '';

    /**
     * @var string Cache expiration period as a string, like '2 hours'
     */
    public string $expires = '';

    /**
     * @var string Scaleway endpoint hostname
     */
    public string $endpoint = '';
    /**
     * @var bool Set ACL for Uploads
     */
    public bool $makeUploadsPublic = true;

    /**
     * @var bool Whether the specified sub folder should be added to the root URL
     */
    public bool $addSubfolderToRootUrl = true;


    public function __construct($config = [])
    {
        parent::__construct($config);

        $this->setConfigs();
    }

    public function setConfigs()
    {
        if ($this->region === '$SCALEWAY_STORAGE_REGION' || $this->region === '') {
            $this->region = Craft::parseEnv('$SCALEWAY_STORAGE_REGION');
        }
        if ($this->bucket === '$SCALEWAY_STORAGE_BUCKET' || $this->bucket === '') {
            $this->bucket = Craft::parseEnv('$SCALEWAY_STORAGE_BUCKET');
        }
        if ($this->keyId === '$SCALEWAY_STORAGE_KEY' || $this->keyId === '') {
            $this->keyId = Craft::parseEnv('$SCALEWAY_STORAGE_KEY');
        }
        if ($this->secret === '$SCALEWAY_STORAGE_SECRET' || $this->secret === '') {
            $this->secret = Craft::parseEnv('$SCALEWAY_STORAGE_SECRET');
        }

        $this->url = 'https://' . $this->bucket . '.s3.' . $this->region . '.scw.cloud/';
        $this->endpoint = 'https://s3.' . $this->region . '.scw.cloud';
    }

    public static function displayName(): string
    {
        return 'Scaleway Object Storage';
    }

    /**
     * @inheritdoc
     */
    protected function defineRules(): array
    {
        return array_merge(parent::defineRules(), [
            [['bucket', 'keyId', 'secret', 'endpoint'], 'required'],
        ]);
    }


    /**
     * @inheritdoc
     */
    public function getSettingsHtml(): ?string
    {
        return Craft::$app->getView()->renderTemplate('scaleway-object-storage/scalewaySettings', [
            'scaleway' => $this,
        ]);
    }

    /**
     * @inheritdoc
     */
    public function getRootUrl(): ?string
    {
        $rootUrl = parent::getRootUrl();

        if ($this->url === '') {
            $rootUrl = 'https://' . $this->bucket . '.s3.' . $this->region . '.scw.cloud/';
        }

        if ($rootUrl && $this->subfolder) {
            $rootUrl .= rtrim($this->subfolder, '/') . '/';
        }

        return $rootUrl;
    }

    /**
     * @inheritdoc
     * @return AwsS3V3Adapter
     */
    protected function createAdapter(): FilesystemAdapter
    {
        $client = static::client($this->getConfigArray());

        return new AwsS3V3Adapter(
            $client,
            App::parseEnv($this->bucket),
            $this->getParsedSubfolder(),
            new PortableVisibilityConverter(),
            new FinfoMimeTypeDetector()
        );
    }

    protected static function client(array $config = []): S3Client
    {
        return new S3Client($config);
    }

    /**
     * @inheritdoc
     */
    protected function addFileMetadataToConfig(array $config): array
    {
        if (DateTimeHelper::isValidIntervalString($this->expires)) {
            $expires = new DateTime();
            $now = new DateTime();
            $expires->modify('+' . $this->expires);
            $diff = (int)$expires->format('U') - (int)$now->format('U');
            $config['CacheControl'] = 'max-age=' . $diff;
        }

        return parent::addFileMetadataToConfig($config);
    }

    /**
     * @return string
     */
    private function getParsedSubfolder(): string
    {
        if ($this->subfolder && ($subfolder = rtrim($this->subfolder, '/')) !== '') {
            return $subfolder . '/';
        }

        return '';
    }


    /**
     * Get the config array for AWS Clients.
     */
    protected function getConfigArray(): array
    {
        return [
            'version' => 'latest',
            'region' => $this->region,
            'endpoint' => $this->endpoint,
            'http_handler' => new GuzzleHandler(Craft::createGuzzleClient()),
            'credentials' => [
                'key' => $this->keyId,
                'secret' => $this->secret,
            ]
        ];
    }


    /**
     * Returns the visibility setting for the Fs.
     *
     * @return string
     */
    protected function visibility(): string
    {
        return $this->makeUploadsPublic ? Visibility::PUBLIC : Visibility::PRIVATE;
    }

    protected function invalidateCdnPath(string $path): bool
    {
        return true;
    }
}
